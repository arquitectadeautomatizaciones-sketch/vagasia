...
import { CheckCircle, Clock, XCircle, Plus, ChevronLeft, ChevronRight } from "lucide-react";
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button className="rounded-lg p-1.5 text-slate-400 hover:bg-white/5 hover:text-white transition-colors">
              <ChevronLeft size={16} />
            </button>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-white">{appt.client?.name}</p>
        ...
        <div className="flex gap-4 text-xs text-slate-500">
          {FILTERS.filter((f) => f.key !== "todas").map((f) => {
            const count = mockAppointments.filter((a) => a.status === f.key).length;
            return (
              <span key={f.key}>
          ...
          <span className="ml-auto text-slate-400">
            Total: €{filtered.reduce((s, a) => s + a.price, 0)}
          </span>
        </div>
      </div>
    </AppLayout>
