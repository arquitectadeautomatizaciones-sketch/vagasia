...
export async function GET() {
  ...
  const { data: businesses, error: bizError } = await db
    .from("businesses")
    .select("id, name, category, phone, email, whatsapp_number, zapi_instance_url, whatsapp_phone_number_id, is_active, created_at, auth_user_id")
  ...
  const professionals = (businesses ?? []).flatMap((b) => {
    ...
    return [{
      ...
      zapi_instance_url: (b as Record<string, unknown>).zapi_instance_url as string ?? null,
